...
class SecretTag(Tag):
    ...
    out = m.form({
        "Password": SecretTag(""),
        "API Key": SecretTag("", show_toggle=True),  # with eye icon to toggle visibility
    })
    ```
    """
    show_toggle: bool = False  # Toggle visibility button (eye icon)
    _masked: bool = True      # Internal state for visibility
    ...
    def _get_ui_val(self):
        """Override UI value representation for masking"""
